...
import type { WorkRequest, Crew, CrewMember, WorkRequestStatus, AcceptanceStatus } from '../../api/types';

const STATUS_LABEL: Record<WorkRequestStatus, string> = {
  REQUESTED: '요청 접수', COMPOSING: '편성 중', PROPOSED: '추천 완료',
  APPROVED: '승인됨', DISPATCHED: '배차 완료', RUNNING: '작업 중',
...
export default function OfficeRequestDetailPage() {
  ...
  const { data: detail } = usePolling<RequestDetail | null>({ fetchFn: fetchDetail, interval: 3000 });
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          ...
          <div className="space-y-2">
            {detail.crew.members.map((member: CrewMember) => {
              ...
              return (
                <div key={member.worker_id} className="flex items-center justify-between text-sm py-2.5 px-3 bg-purple-50 rounded">
                  <div className="flex items-center gap-3">
                    <span className="font-medium text-gray-800">{member.name}</span>
                    <span className="text-xs text-gray-500">{TRADE_LABEL[member.assigned_trade]}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-500">{member.offered_wage.toLocaleString()}원</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${accInfo.color}`}>
                      {accInfo.label}
                    </span>
                  </div>
                </div>
